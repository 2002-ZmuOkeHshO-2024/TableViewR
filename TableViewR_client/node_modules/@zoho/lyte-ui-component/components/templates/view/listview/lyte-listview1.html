<template tag-name="lyte-listview1">
	...
	<%if(ltPropFilters && renderFilter){%>
		<lyte-list-filter lt-prop = {{ltPropFilter}} lt-prop-format = {{ltPropFormat}} lt-prop-conditions = {{lbind(ltPropConditions)}} lt-prop-any = {{lbind(ltPropAny)}} lt-prop-show = {{lbind(ltPropShowFilters)}} lt-prop-fields = {{headerData}} lt-prop-content = {{ltPropContent}} lt-prop-filtered-content="{{lbind(ltPropFilteredContent)}}" lt-prop-render-content="{{lbind(ltPropRenderContent)}}" lt-prop-modal-properties="{{ltPropModalProperties}}" lt-prop-filter-element-properties="{{ltPropFilterElementProperties}}" on-condition-change = "{{method('filter')}}" on-picklist-construct = "{{method('picklistData')}}" fetch-more-data = "{{method('fetchData')}}" on-custom-filter-reset = "{{method('customReset')}}" on-before-filter-open = "{{method('beforeOpen')}}" on-before-filter-close = "{{method('beforeClose')}}" on-modal-filter-apply="{{method('onModalApply')}}" on-modal-filter-remove="{{method('onModalFilterRemove')}}" on-reset="{{method('onBeforeReset')}}" on-apply="{{method('onBeforeApply')}}" on-cancel="{{method('onBeforeCancel')}}">
